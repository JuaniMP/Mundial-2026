...
const STADIUMS: Stadium[] = [
  { id: 1,  nombre: 'MetLife Stadium',      ciudad: 'East Rutherford, NJ', pais: 'USA',    capacidad: 82500  },
  ...
  { id: 11, nombre: 'Hard Rock Stadium',    ciudad: 'Miami Gardens, FL',   pais: 'USA',    capacidad: 65326  },
  ...
  { id: 14, nombre: 'Estadio Azteca',       ciudad: 'Ciudad de México',    pais: 'México', capacidad: 87523  },
...
function AlbumCover({ laminas, onOpen }: { laminas: LaminaAlbum[]; onOpen: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Gold lines */}
      <div style={{ position: 'absolute', top: 14, left: 14, right: 14, height: 2, background: 'linear-gradient(90deg,transparent,#fbbf24,transparent)' }} />
      <div style={{ position: 'absolute', bottom: 14, left: 14, right: 14, height: 2, background: 'linear-gradient(90deg,transparent,#fbbf24,transparent)' }} />

      {/* Host flags */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, fontSize: 28, filter: 'drop-shadow(0 2px 6px rgba(0,0,0,0.5))' }}>
        🇺🇸 🇲🇽 🇨🇦
      </div>

      {/* Trophy */}
      <div style={{ fontSize: 56, marginBottom: 8, filter: 'drop-shadow(0 4px 20px rgba(251,191,36,0.5))', animation: 'float 3s ease-in-out infinite' }}>
        🏆
      </div>
      ...
      <h1 style={{
        ...
        color: '#fbbf24', textAlign: 'center', margin: 0,
      ...
      <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, letterSpacing: 4, textTransform: 'uppercase', fontFamily: 'Inter,sans-serif', marginBottom: 20 }}>
      ...
      <div style={{ width: '60%', maxWidth: 200 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10, fontFamily: 'Inter,sans-serif' }}>{pasted} pegados</span>
          <span style={{ color: '#fbbf24', fontSize: 10, fontFamily: 'Oswald,sans-serif', fontWeight: 700 }}>{pct}%</span>
        </div>
        <div style={{ height: 4, borderRadius: 99, background: 'rgba(255,255,255,0.1)', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${pct}%`, background: 'linear-gradient(90deg,#1d4ed8,#fbbf24)', borderRadius: 99, transition: 'width 0.6s ease' }} />
        </div>
      </div>
...
// ── Page wrapper ──
function BookPage({ children, pageNum, isLeft }: { children: React.ReactNode; pageNum?: number; isLeft: boolean }) {
  return (
    <div style={{
      flex: 1,
      background: '#faf7f0',
...
// ── Slot in a team page ──
function AlbumSlot({
  slotIndex, teamIdx, laminaData, onPegar,
}: {
  slotIndex: number;
  teamIdx: number;
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 9, color: '#94a3b8', fontFamily: 'Inter,sans-serif' }}>{String(laminaId).padStart(3,'0')}</div>
        </div>
        <div style={{ position:'absolute', bottom:3, right:4, fontSize:9, color:'#cbd5e1', fontFamily:'Inter,sans-serif' }}>#{slotNum}</div>
      </div>
    );
  }
...
// ── Team page ──
function TeamPage({
  team, teamIdx, laminas, onPegar, isLeft, pageNum,
}: {
  team: Team; teamIdx: number; laminas: LaminaAlbum[];
  onPegar: (id: number) => void; isLeft: boolean; pageNum: number;
}) {
  const slots = getLaminasForTeam(laminas, teamIdx);
  ...
  return (
    <BookPage isLeft={isLeft} pageNum={pageNum}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 22, lineHeight: 1, filter: 'drop-shadow(0 1px 3px rgba(0,0,0,0.4))' }}>{team.flag}</span>
        <div style={{ flex: 1 }}>
          <p style={{ color: '#fff', fontFamily: 'Oswald,sans-serif', fontWeight: 700, fontSize: 13, margin: 0, lineHeight: 1, textTransform: 'uppercase', letterSpacing: 0.5 }}>
            {team.name}
          </p>
          <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 9, margin: '2px 0 0', fontFamily: 'Inter,sans-serif' }}>
      ...
      <div style={{ height: 3, background: 'rgba(0,0,0,0.06)' }}>
        <div style={{ height: '100%', width: `${pct}%`, background: team.primary, transition: 'width 0.5s ease' }} />
      </div>
  ...
  return (
    <BookPage isLeft={isLeft} pageNum={pageNum}>
      <div style={{
        ...
        <p style={{ color: '#fff', fontFamily: 'Oswald,sans-serif', fontWeight: 700, fontSize: 14, margin: 0, textTransform: 'uppercase', letterSpacing: 1 }}>
          🏟️ Estadios FIFA WC 2026
        </p>
        <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 9, margin: '3px 0 0', fontFamily: 'Inter,sans-serif' }}>
      ...
      <div style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        {['USA', 'Canadá', 'México'].map(pais => {
          ...
          return (
            <div key={pais}>
              <p style={{ color: '#64748b', fontSize: 9, fontFamily: 'Inter,sans-serif', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, margin: '6px 0 3px' }}>
              ...
              {list.map(s => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '2px 4px' }}>
                  <span style={{ fontSize: 10, color: '#334155', fontFamily: 'Inter,sans-serif' }}>{s.nombre}</span>
                  <span style={{ fontSize: 9, color: '#94a3b8', fontFamily: 'Inter,sans-serif' }}>{(s.capacidad/1000).toFixed(0)}k</span>
                </div>
              ))}
            </div>
          );
...
function StadiumPage({
  stadium, isLeft, pageNum,
}: {
  stadium: Stadium; isLeft: boolean; pageNum: number;
}) {
  const clr = stadium.pais === 'USA' ? '#002868' : stadium.pais === 'México' ? '#006847' : '#CC0000';
  return (
    <BookPage isLeft={isLeft} pageNum={pageNum}>
      <div style={{ background: `linear-gradient(135deg,${clr} 0%,#4c1d95 100%)`, padding: '10px 12px 8px' }}>
        <p style={{ color: '#fff', fontFamily: 'Oswald,sans-serif', fontWeight: 700, fontSize: 13, margin: 0, textTransform: 'uppercase', letterSpacing: 0.5 }}>
          🏟️ {stadium.nombre}
        </p>
        <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 9, margin: '2px 0 0', fontFamily: 'Inter,sans-serif' }}>
      ...
      <div style={{ flex: 1, padding: '12px', display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          {[
            ...
            <div key={k} style={{
              ...
              <p style={{ color: '#94a3b8', fontSize: 8, margin: 0, fontFamily: 'Inter,sans-serif', textTransform: 'uppercase', letterSpacing: 0.5 }}>{k}</p>
              <p style={{ color: '#1e293b', fontSize: 10, margin: '1px 0 0', fontFamily: 'Oswald,sans-serif', fontWeight: 600 }}>{v}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  if (!page) {
    return (
      <BookPage isLeft={isLeft} pageNum={undefined}>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#e2e8f0', fontSize: 40 }}>
          ⚽
        </div>
      </BookPage>
    );
  ...
  return (
    <button
      ...
      style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: 20, color: disabled ? '#e2e8f0' : '#475569',
...
export function AlbumBook({ laminas, onPegar }: AlbumBookProps) {
  ...
  const animClass = flipDir === 'next' ? 'book-flip-next' : flipDir === 'prev' ? 'book-flip-prev' : undefined;

  // Jump options
  const jumpOptions: { label: string; spread: number }[] = [
    { label: '📖 Portada', spread: 0 },
    ...TEAMS.map((t, i) => ({ label: `${t.flag} ${t.name} (Gr.${t.group})`, spread: 1 + Math.floor(i / 2) })),
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        >
          {spreadIdx === 0 ? (
            ...
            <>
              ...
              {/* Spine */}
              <div style={{ width: 16, background: 'linear-gradient(90deg,#c8b99a,#e8dfc8,#c8b99a)', flexShrink: 0 }} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
        ...
        <span style={{ fontSize: 12, color: '#64748b', fontFamily: 'Inter,sans-serif', whiteSpace: 'nowrap' }}>
          {spreadIdx === 0 ? 'Portada' : `${(spreadIdx - 1) * 2 + 1}–${Math.min((spreadIdx - 1) * 2 + 2, CONTENT_PAGES.length)}`} / {CONTENT_PAGES.length}
        </span>
