...
const MISSIONS: Mission[] = [
  ...
  },
  {
    ...
    reward: 60,
    type: 'rarity',
    check: ls => ({ progress: Math.min(ls.filter(l => parseRareza(l.rareza) === 'RARO').length, 3), goal: 3 }),
  },
  {
    id: 'epico-1',
    icon: '💜',
    ...
    reward: 100,
    type: 'rarity',
    check: ls => ({ progress: Math.min(ls.filter(l => parseRareza(l.rareza) === 'EPICO').length, 1), goal: 1 }),
  },
  {
    id: 'legendario-1',
    icon: '⭐',
    ...
    reward: 250,
    type: 'rarity',
    check: ls => ({ progress: Math.min(ls.filter(l => parseRareza(l.rareza) === 'LEGENDARIO').length, 1), goal: 1 }),
  },
  {
    id: 'team-complete',
    icon: '🏆',
  ...
  },
  {
    ...
    check: ls => {
      ...
      const count = ls.filter(l => hostShorts.includes(getTeamForLamina(l.idLamina).shortName)).length;
...
export function Missions({ laminas, onClaim }: MissionsProps) {
  ...
  const totalReward = evaluated.filter(m => m.completed && !m.isClaimed).reduce((s, m) => s + m.reward, 0);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '8px 14px' }}>
          <span style={{ fontSize: 20, fontWeight: 800, color: '#60a5fa', fontFamily: 'Oswald,sans-serif' }}>{completedCount}</span>
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', fontFamily: 'Inter,sans-serif', display: 'block' }}>/ {MISSIONS.length} Completadas</span>
        </div>
        <div style={{ background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.25)', borderRadius: 10, padding: '8px 14px' }}>
          <span style={{ fontSize: 20, fontWeight: 800, color: '#fbbf24', fontFamily: 'Oswald,sans-serif' }}>{coins} 🪙</span>
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', fontFamily: 'Inter,sans-serif', display: 'block' }}>Tu saldo</span>
        </div>
        {totalReward > 0 && (
          <div style={{ background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.3)', borderRadius: 10, padding: '8px 14px' }}>
            <span style={{ fontSize: 20, fontWeight: 800, color: '#34d399', fontFamily: 'Oswald,sans-serif' }}>+{totalReward}</span>
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', fontFamily: 'Inter,sans-serif', display: 'block' }}>Pendientes de cobro</span>
          </div>
        )}
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{
                ...
                background: `${clr}22`, border: `1px solid ${clr}44`,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                  <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, margin: 0, fontFamily: 'Oswald,sans-serif', letterSpacing: 0.5 }}>
                    {m.title}
                  </p>
                  <span style={{ color: '#fbbf24', fontSize: 12, fontWeight: 700, fontFamily: 'Oswald,sans-serif', flexShrink: 0 }}>
                    +{m.reward} 🪙
                  </span>
                </div>
                <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 12, margin: '3px 0 8px', fontFamily: 'Inter,sans-serif' }}>
                  {m.description}
                </p>
                {/* Progress bar */}
                ...
                <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11, margin: '4px 0 0', fontFamily: 'Inter,sans-serif' }}>
                  {m.progress} / {m.goal}
                </p>
              </div>
              ...
              {m.isClaimed && (
                <span style={{ fontSize: 12, color: '#34d399', fontFamily: 'Inter,sans-serif', flexShrink: 0 }}>Cobrado</span>
              )}
            </div>
          );
        })}
