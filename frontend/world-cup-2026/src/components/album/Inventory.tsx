...
export function Inventory({ laminas, onPegar, onVender }: InventoryProps) {
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortBy === 'rarity')
        return (
          (RARITY_ORDER[parseRareza(a.rareza)] ?? 9) - (RARITY_ORDER[parseRareza(b.rareza)] ?? 9)
        );
      if (sortBy === 'team')
        return getTeamForLamina(a.idLamina).name.localeCompare(getTeamForLamina(b.idLamina).name);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '4px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        ...
        {(['COMUN', 'RARO', 'EPICO', 'LEGENDARIO'] as const).map((rz) =>
          pill(filterRareza === rz, RARITY[rz].label, () =>
            setFilterRareza(filterRareza === rz ? 'all' : rz),
          ),
        )}
      </div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        ...
        {pill(filterEstado === 'pegadas', 'Pegadas', () =>
          setFilterEstado(filterEstado === 'pegadas' ? 'all' : 'pegadas'),
        )}
        {pill(filterEstado === 'no-pegadas', 'Sin pegar', () =>
          setFilterEstado(filterEstado === 'no-pegadas' ? 'all' : 'no-pegadas'),
        )}
        {pill(filterEstado === 'repetidas', 'Repetidas', () =>
          setFilterEstado(filterEstado === 'repetidas' ? 'all' : 'repetidas'),
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', maxHeight: 72, overflowY: 'auto' }}>
        ...
        {TEAMS.map((t) =>
          pill(filterTeam === t.shortName, `${t.flag} ${t.shortName}`, () =>
            setFilterTeam(filterTeam === t.shortName ? 'all' : t.shortName),
          ),
        )}
      </div>
