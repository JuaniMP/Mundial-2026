          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <button
                  type="button"
                  onClick={() => { setTeamOpen(o => !o); touch('team'); }}
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 text-text-muted transition-transform ${teamOpen ? 'rotate-180' : ''}`} />
                </button>
                            ...
                            <div key={grp}>
                              ...
                              {inGroup.map(t => (
                                <button
                                  key={t.shortName}
                                  type="button"
                                  onClick={() => { setSeleccion(t.shortName); setTeamOpen(false); setTeamSearch(''); }}
                                ...
                                >
                                  ...
                                  {seleccion === t.shortName && <Check className="w-3.5 h-3.5 text-primary" />}
                                </button>
                              ))}
                            </div>
                          );
                ...
                <div className="mt-2 space-y-1.5">
                  ...
                  <div className="grid grid-cols-1 gap-0.5">
                    {strength.rules.map(r => (
                      <p key={r.text} className={`text-xs flex items-center gap-1.5 ${r.ok ? 'text-green-600' : 'text-text-muted'}`}>
                        {r.ok
            ...
            {/* ── Confirm password ── */}
            <div>
              <label htmlFor="confirmPwd" className="block text-sm font-semibold text-text-primary mb-1.5">
              ...
              {confirmPassword.length > 0 && (
                <p className={`mt-1 text-xs flex items-center gap-1 ${pwMatch ? 'text-green-600' : 'text-accent'}`}>
                  {pwMatch
